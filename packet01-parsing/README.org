...
trigger a tracepoint event (=xdp:xdp_exception=), which have zero-overhead
when not enabled. =PASS= will allow the packet to continue up to the kernel
networking stack for processing, =TX= will retransmit the packet out of the
same interface it was received on, and =REDIRECT= will transmit the packet
out of another interface (where destination interface is set by BPF-helper
call prior to returning =REDIRECT=).
...
convert to and from host byte order, respectively. (Detail of why you should
use these =bpf_= prefixed byteorder(3) functions are described in the header
file that defines them file:../headers/bpf_endian.h).
...
*Gotcha:* XDP need to see the VLAN-headers *inline* in the packet. Most NIC
drivers support VLAN offloading, where the VLAN-info is *not* inline, but
instead deliver the VLAN-info in the hardware descriptor (and transfer info
to SKB related vlan fields). Thus, make sure that this vlan-offload feature
is turned off, via ethtool:

#+begin_example
 # Check current setting:
 ethtool -k veth-xdp | grep vlan-offload
